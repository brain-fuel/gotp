...
func pinnedQueueModule(t *testing.T) *LoadedModule {
	...
	match LoadModuleFile(read, "../beam/testdata/otp-29.0.4/queue.beam", ModuleLoaderConfig{}) {
...
func TestPinnedOTPQueueCorpusCoversEveryExport(t *testing.T) {
	...
	for target := range image.Exports {
		...
		if !covered[identity] { t.Errorf("pinned queue export lacks differential case: %s", identity) }
	}
}
...
func invokeQueueCorpusCase(t *testing.T, modules *ModuleSet, registry *CallRegistry, function string, arguments []term.Term) *VMProcess {
	...
	match modules.Invoke("queue", function, arguments, clock.Real{}, registry) {
	case result.Err(failure): t.Fatal(failure)
...
func assertQueueCorpusOutcome(t *testing.T, function string, arity int, actual VMProcessState, expected term.Term) {
	...
	case term.TupleTerm(parts):
		...
		case term.AtomTerm(tag):
			...
			case "ok":
				match actual {
				case VMProcessCompleted(value, _, _): if !queueCorpusValuesEqual(function, arity, value, parts[1]) { t.Fatalf("%s/%d = %v, want %v", function, arity, value, parts[1]) }
...
func queueCorpusValuesEqual(function string, arity int, actual term.Term, expected term.Term) bool {
	...
	if arity == 1 { return unorderedQueueCorpusTermsEqual(actual, expected) }
	match actual {
		...
		case term.ProperListTerm(expectedFields):
			...
			for _, actualField := range actualFields {
				...
				for index, expectedField := range expectedFields {
					if !matched[index] && queueCorpusModuleInfoFieldEqual(actualField, expectedField) { matched[index] = true; found = true; break }
				}
...
func queueCorpusModuleInfoFieldEqual(actual term.Term, expected term.Term) bool {
	...
	case term.TupleTerm(actualParts):
		...
		case term.TupleTerm(expectedParts):
			...
			if term.Equal(actualParts[0], term.MustAtom("exports")) { return unorderedQueueCorpusTermsEqual(actualParts[1], expectedParts[1]) }
