// Code generated by goplus from otp_ordsets_differential_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func pinnedOrdsetsModule(t *testing.T) *LoadedModule {
	...
	switch __gp_m0 := any(LoadModuleFile(read, "../beam/testdata/otp-29.0.4/ordsets.beam", ModuleLoaderConfig{})).(type) {
...
func TestPinnedOTPOrdsetsCorpusCoversEveryExport(t *testing.T) {
	...
	for target := range image.Exports {
		...
		if !covered[identity] {
			t.Errorf("pinned ordsets export lacks differential case: %s", identity)
		}
	}
}
...
func invokeOrdsetsCorpusCase(t *testing.T, modules *ModuleSet, registry *CallRegistry, function string, arguments []term.Term) *VMProcess {
	...
	switch __gp_m3 := any(modules.Invoke("ordsets", function, arguments, clock.Real{}, registry)).(type) {
		...
		case term.AtomTerm:
			...
			case "ok":
				...
				case VMProcessCompleted:
					value := __gp_m7.Value
					if !ordsetsCorpusValuesEqual(function, arity, value, parts[1]) {
...
func ordsetsCorpusValuesEqual(function string, arity int, actual term.Term, expected term.Term) bool {
	...
	if arity == 1 {
		return unorderedOrdsetsCorpusTermsEqual(actual, expected)
	}
		...
		case term.ProperListTerm:
			...
			for _, actualField := range actualFields {
				...
				for index, expectedField := range expectedFields {
					if !matched[index] && ordsetsCorpusModuleInfoFieldEqual(actualField, expectedField) {
						matched[index] = true
						found = true
						break
					}
	...
	case term.TupleTerm:
		...
		case term.TupleTerm:
			...
			if term.Equal(actualParts[0], term.MustAtom("exports")) {
				return unorderedOrdsetsCorpusTermsEqual(actualParts[1], expectedParts[1])
			}
