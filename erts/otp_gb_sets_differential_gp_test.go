// Code generated by goplus from otp_gb_sets_differential_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func pinnedGbSetsModule(t *testing.T) *LoadedModule {
	...
	switch __gp_m0 := any(LoadModuleFile(read, "../beam/testdata/otp-29.0.4/gb_sets.beam", ModuleLoaderConfig{})).(type) {
...
func TestPinnedOTPGbSetsDifferentialCorpus(t *testing.T) {
	...
	switch __gp_m1 := any(NewModuleSet([]*LoadedModule{module, pinnedListsModule(t), pinnedStdlibDependency(t, "ordsets")})).(type) {
...
func TestPinnedOTPGbSetsCorpusCoversEveryExport(t *testing.T) {
	...
	for target := range image.Exports {
		...
		if !covered[identity] {
			t.Errorf("pinned gb_sets export lacks differential case: %s", identity)
		}
	}
}
...
func invokeGbSetsCorpusCase(t *testing.T, modules *ModuleSet, registry *CallRegistry, function string, arguments []term.Term) *VMProcess {
	...
	switch __gp_m3 := any(modules.Invoke("gb_sets", function, arguments, clock.Real{}, registry)).(type) {
		...
		case term.AtomTerm:
			...
			case "ok":
				...
				case VMProcessCompleted:
					value := __gp_m7.Value
					if !gb_setsCorpusValuesEqual(function, arity, value, parts[1]) {
...
func gb_setsCorpusValuesEqual(function string, arity int, actual term.Term, expected term.Term) bool {
	...
	if arity == 1 {
		return unorderedGbSetsCorpusTermsEqual(actual, expected)
	}
		...
		case term.ProperListTerm:
			...
			for _, actualField := range actualFields {
				...
				for index, expectedField := range expectedFields {
					if !matched[index] && gb_setsCorpusModuleInfoFieldEqual(actualField, expectedField) {
						matched[index] = true
						found = true
						break
					}
	...
	case term.TupleTerm:
		...
		case term.TupleTerm:
			...
			if term.Equal(actualParts[0], term.MustAtom("exports")) {
				return unorderedGbSetsCorpusTermsEqual(actualParts[1], expectedParts[1])
			}
