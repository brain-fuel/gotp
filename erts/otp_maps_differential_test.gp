...
func pinnedMapsModule(t *testing.T) *LoadedModule {
	var read beam.ReadFileCapability = beam.OperatingSystemFiles{}
	match LoadModuleFile(read, "../beam/testdata/otp-29.0.4/maps.beam", ModuleLoaderConfig{}) {
...
// assayxport:law gotp.erts.otp29-maps-differential
// assayxport:law gotp.erts.otp29-maps-differential
func TestPinnedOTPMapsDifferentialCorpus(t *testing.T) {
	payload, cause := os.ReadFile("testdata/otp-29.0.4-maps.corpus")
	if cause != nil { t.Fatal(cause) }
	module := pinnedMapsModule(t)
	...
	for lineNumber, line := range strings.Split(strings.TrimSpace(string(payload)), "\n") {
		...
		if cause != nil { t.Fatal(cause) }
		argumentsTerm := decodeMapsCorpusTerm(t, fields[3])
		...
		expected := decodeMapsCorpusTerm(t, fields[4])
		process := invokeMapsCorpusCase(t, modules, registry, fields[1], arguments)
		assertMapsCorpusOutcome(t, fields[1], arity, process.State(), expected)
	}
}

// assayxport:law gotp.erts.otp29-maps-export-coverage
func TestPinnedOTPMapsCorpusCoversEveryExport(t *testing.T) {
	covered := map[string]bool{}
	for _, corpus := range []string{"testdata/otp-29.0.4-maps.corpus", "testdata/otp-29.0.4-maps-callbacks.corpus"} {
	...
	image := pinnedMapsModule(t).image()
	for target := range image.Exports {
		identity := target.Function + "/" + strconv.Itoa(int(target.Arity))
		if !covered[identity] { t.Errorf("pinned maps export lacks differential case: %s", identity) }
	}
}

func invokeMapsCorpusCase(t *testing.T, modules *ModuleSet, registry *CallRegistry, function string, arguments []term.Term) *VMProcess {
	t.Helper()
	var process *VMProcess
	match modules.Invoke("maps", function, arguments, clock.Real{}, registry) {
	case result.Err(failure): t.Fatal(failure)
...
func runMapsCorpusProcess(t *testing.T, process *VMProcess) {
	t.Helper()
...
func assertMapsCorpusOutcome(t *testing.T, function string, arity int, actual VMProcessState, expected term.Term) {
	t.Helper()
	match expected {
	case term.TupleTerm(parts):
		...
		case term.AtomTerm(tag):
			...
			case "ok":
				match actual {
				case VMProcessCompleted(value, _, _): if !mapsCorpusValuesEqual(function, arity, value, parts[1]) { t.Fatalf("%s/%d = %v, want %v", function, arity, value, parts[1]) }
...
func mapsCorpusValuesEqual(function string, arity int, actual term.Term, expected term.Term) bool {
	if function != "module_info" { return term.Equal(actual, expected) }
	if arity == 1 { return unorderedMapsCorpusTermsEqual(actual, expected) }
	match actual {
		...
		case term.ProperListTerm(expectedFields):
			...
			for _, actualField := range actualFields {
				...
				for index, expectedField := range expectedFields {
					if !matched[index] && mapsCorpusModuleInfoFieldEqual(actualField, expectedField) { matched[index] = true; found = true; break }
				}
...
func mapsCorpusModuleInfoFieldEqual(actual term.Term, expected term.Term) bool {
	match actual {
	case term.TupleTerm(actualParts):
		...
		case term.TupleTerm(expectedParts):
			...
			if term.Equal(actualParts[0], term.MustAtom("exports")) { return unorderedMapsCorpusTermsEqual(actualParts[1], expectedParts[1]) }
...
func unorderedMapsCorpusTermsEqual(actual term.Term, expected term.Term) bool {
	match actual {
...
func decodeMapsCorpusTerm(t *testing.T, encoded string) term.Term {
	t.Helper()
