...
func pinnedGbTreesModule(t *testing.T) *LoadedModule {
	var read beam.ReadFileCapability = beam.OperatingSystemFiles{}
	match LoadModuleFile(read, "../beam/testdata/otp-29.0.4/gb_trees.beam", ModuleLoaderConfig{}) {
...
// assayxport:law gotp.erts.otp29-gb_trees-differential
// assayxport:law gotp.erts.otp29-gb_trees-differential
func TestPinnedOTPGbTreesDifferentialCorpus(t *testing.T) {
	payload, cause := os.ReadFile("testdata/otp-29.0.4-gb_trees.corpus")
	if cause != nil { t.Fatal(cause) }
	module := pinnedGbTreesModule(t)
	var modules *ModuleSet
	match NewModuleSet([]*LoadedModule{module, pinnedListsModule(t), pinnedStdlibDependency(t, "orddict")}) { case result.Err(failure): t.Fatal(failure); case result.Ok(found): modules = found }
	...
	for lineNumber, line := range strings.Split(strings.TrimSpace(string(payload)), "\n") {
		...
		if cause != nil { t.Fatal(cause) }
		argumentsTerm := decodeGbTreesCorpusTerm(t, fields[3])
		...
		expected := decodeGbTreesCorpusTerm(t, fields[4])
		process := invokeGbTreesCorpusCase(t, modules, registry, fields[1], arguments)
		assertGbTreesCorpusOutcome(t, fields[1], arity, process.State(), expected)
	}
}

// assayxport:law gotp.erts.otp29-gb_trees-export-coverage
func TestPinnedOTPGbTreesCorpusCoversEveryExport(t *testing.T) {
	covered := map[string]bool{}
	for _, corpus := range []string{"testdata/otp-29.0.4-gb_trees.corpus", "testdata/otp-29.0.4-gb_trees-callbacks.corpus"} {
	...
	image := pinnedGbTreesModule(t).image()
	for target := range image.Exports {
		identity := target.Function + "/" + strconv.Itoa(int(target.Arity))
		if !covered[identity] { t.Errorf("pinned gb_trees export lacks differential case: %s", identity) }
	}
}

func invokeGbTreesCorpusCase(t *testing.T, modules *ModuleSet, registry *CallRegistry, function string, arguments []term.Term) *VMProcess {
	t.Helper()
	var process *VMProcess
	match modules.Invoke("gb_trees", function, arguments, clock.Real{}, registry) {
	case result.Err(failure): t.Fatal(failure)
...
func runGbTreesCorpusProcess(t *testing.T, process *VMProcess) {
	t.Helper()
...
func assertGbTreesCorpusOutcome(t *testing.T, function string, arity int, actual VMProcessState, expected term.Term) {
	t.Helper()
	match expected {
	case term.TupleTerm(parts):
		...
		case term.AtomTerm(tag):
			...
			case "ok":
				match actual {
				case VMProcessCompleted(value, _, _): if !gb_treesCorpusValuesEqual(function, arity, value, parts[1]) { t.Fatalf("%s/%d = %v, want %v", function, arity, value, parts[1]) }
...
func gb_treesCorpusValuesEqual(function string, arity int, actual term.Term, expected term.Term) bool {
	if function != "module_info" { return term.Equal(actual, expected) }
	if arity == 1 { return unorderedGbTreesCorpusTermsEqual(actual, expected) }
	match actual {
		...
		case term.ProperListTerm(expectedFields):
			...
			for _, actualField := range actualFields {
				...
				for index, expectedField := range expectedFields {
					if !matched[index] && gb_treesCorpusModuleInfoFieldEqual(actualField, expectedField) { matched[index] = true; found = true; break }
				}
...
func gb_treesCorpusModuleInfoFieldEqual(actual term.Term, expected term.Term) bool {
	match actual {
	case term.TupleTerm(actualParts):
		...
		case term.TupleTerm(expectedParts):
			...
			if term.Equal(actualParts[0], term.MustAtom("exports")) { return unorderedGbTreesCorpusTermsEqual(actualParts[1], expectedParts[1]) }
...
func unorderedGbTreesCorpusTermsEqual(actual term.Term, expected term.Term) bool {
	match actual {
...
func decodeGbTreesCorpusTerm(t *testing.T, encoded string) term.Term {
	t.Helper()
