// Code generated by goplus from otp_gen_statem_inventory_audit_test.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
type genStatemAuditEvidence struct {
...
type genStatemAuditWire struct {
	Items []genStatemAuditItem `json:"items"`
}
...
func TestGenStatemCompleteInventoryAudit(t *testing.T) {
	...
	for _, item := range wire.Items {
		if !strings.Contains(item.ID, ".stdlib.gen_statem.") || !strings.HasPrefix(item.ID, "otp.declaration.") {
			continue
		}
		parts := strings.Split(item.ID, ".")
		if len(parts) < 4 {
			t.Fatalf("malformed declaration identity %s", item.ID)
		}
		kind := parts[2]
		counts[kind]++
		if kind == "type" {
	...
	for kind, wanted := range expected {
		if counts[kind] != wanted {
			t.Fatalf("gen_statem %s declarations = %d, want %d", kind, counts[kind], wanted)
		}
	}
}
