...
func TestPinnedOTPGenServerRemainingRuntime(t *testing.T) {
    ...
    for lineNumber, line := range strings.Split(strings.TrimSpace(string(payload)), "\n") {
        ...
        match modules.Invoke("gen_server_remaining_callbacks", fields[1], []term.Term{}, clock.Real{}, registry) { case result.Err(failure): t.Fatal(failure); case result.Ok(created): process = created }
...
func pinnedGenServerRemainingModules(t *testing.T) *ModuleSet {
    ...
    match LoadModuleFile(read, "testdata/otp-29.0.4/gen_server_remaining_callbacks.beam", ModuleLoaderConfig{}) { case result.Err(failure): t.Fatal(failure); case result.Ok(found): loaded = append(loaded, found) }
