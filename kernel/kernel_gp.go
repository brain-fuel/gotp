...
func (kernel *Kernel) Send(
	...
	if to.Node != kernel.config.Node {
		if kernel.config.Remote == nil {
			return NoProcess{}
		}
		return kernel.config.Remote.SendPID(from, to, term.Clone(message))
	}
...
func (kernel *Kernel) SendRemoteRegistered(from term.PID, node string, name string, message term.Term) Delivery {
	if node == kernel.config.NodeName {
		return kernel.SendRegistered(from, name, message)
	}
	if kernel.config.Remote == nil {
		return NoProcess{}
	}
	return kernel.config.Remote.SendName(from, node, name, term.Clone(message))
}
...
func (kernel *Kernel) MonitorTaggedName(watcher term.PID, node string, name string, tag term.Term) result.Result[term.Reference, Failure] {
	...
	case option.Some[*process]:
		...
		current.remoteMonitors[reference] = remoteMonitor{node: node, name: name, tag: term.Clone(tag)}
...
func (kernel *Kernel) enqueueTaggedDown(watcher term.PID, source term.PID, reference term.Reference, object term.Term, reason term.Term, tag term.Term) Delivery {
	...
	return kernel.enqueueSignal(watcher, UserSignal{From: source, Sequence: 0, Message: term.Tuple(term.Clone(tag), term.ReferenceValue(reference), term.MustAtom("process"), term.Clone(object), term.Clone(reason))})
}
...
func (kernel *Kernel) Demonitor(
	...
	case option.Some[*process]:
		...
		if remoteExists {
			...
			if kernel.config.Remote != nil {
				kernel.config.Remote.DemonitorName(watcher, remote.node, remote.name, reference)
			}
		}
...
func (kernel *Kernel) terminate(current *process, reason term.Term) {
	...
	for _, reference := range references {
		...
		case option.Some[*process]:
			...
			if found, present := watcherProcess.monitorTags[reference]; present {
				tag = term.Clone(found)
				delete(watcherProcess.monitorTags, reference)
			}
			name, named := current.monitorNames[reference]
			if named {
...
func (context *Context) NodeNameFor(node uint32) option.Option[string] {
	if node == context.kernel.config.Node {
		return option.Some[string]{Value: context.kernel.config.NodeName}
	}
	if context.kernel.config.Remote == nil {
...
func (context *Context) MonitorTagged(other term.PID, tag term.Term) result.Result[term.Reference, Failure] {
	return context.kernel.MonitorTagged(context.process.pid, other, tag)
}
func (context *Context) MonitorTaggedName(node string, name string, tag term.Term) result.Result[term.Reference, Failure] {
	return context.kernel.MonitorTaggedName(context.process.pid, node, name, tag)
}
